/* settings.js v19 - ПРУТКОН ERP Complete System Overhaul */
...
// История изменений с уникальными ID
window.settingsChangeLog = [];
window.logID = 0;

// Текущая отредактированная карточка
...
// === ТАБЛИЦА СОТРУДНИКОВ (упрощённая) ===
function renderEmpSettings() {
    ...
    if (!window.dbEmployees || window.dbEmployees.length === 0) return;
    ...
    window.dbEmployees.forEach((emp, index) => {
        ...
        tr.innerHTML = `
            ...
            <td style="text-align:right;">
                <button class="action-btn" title="Редактировать" onclick="window.openEmployeeCard(${index})"><i class="fa-solid fa-edit"></i></button>
                <button class="action-btn" title="Удалить" onclick="window.delEmp(${index})"><i class="fa-solid fa-trash" style="color:#ff1e27;"></i></button>
            </td>
        `;
...
// === КАРТОЧКА СОТРУДНИКА (модальное окно) ===
window.openEmployeeCard = (index) => {
    if (!window.dbEmployees[index]) return;
    ...
    const html = `
        <div class="panel glass-panel" style="width: 500px; max-height: 85vh; overflow-y: auto;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
                ...
                <button onclick="document.getElementById('emp-card-modal').style.display='none'" style="background:none; border:none; font-size:1.5rem; cursor:pointer; color:#888;">✕</button>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                ...
                <div class="form-group">
                    <label>% ФОТ</label>
                    <input type="number" id="emp-share" class="form-control" value="${(emp.share * 100).toFixed(0)}" style="height:40px;" placeholder="%">
                </div>
            </div>
            
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                <div class="form-group">
                    <label>Пароль (по умолчанию 623401)</label>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 25px;">
                <button class="btn btn-secondary" onclick="window.closeEmployeeCard()"><i class="fa-solid fa-xmark"></i> Отмена</button>
                <button class="btn btn-primary" onclick="window.saveEmployeeCard()"><i class="fa-solid fa-check"></i> Сохранить</button>
            </div>
        </div>
    `;
...
window.closeEmployeeCard = () => {
    document.getElementById('emp-card-modal').style.display = 'none';
    window.editingEmployeeIndex = null;
};

window.saveEmployeeCard = () => {
    if (window.editingEmployeeIndex === null) return;
    ...
    // Логируем изменения
    logChange('emp_card_update', {
        index: idx,
        name: emp.name,
        changes: {
            oldRole: oldData.role,
            newRole: emp.role,
            oldSalary: oldData.base,
            newSalary: emp.base,
            status: emp.status
        }
    });
...
window.addEmployee = () => {
    ...
    const newEmp = { 
        id: Math.max(...window.dbEmployees.map(e => e.id || 0)) + 1,
...
window.delEmp = (idx) => {
    ...
    if(confirm('🗑️ Удалить сотрудника? Это действие нельзя отменить!')) {
...
function highlightSaveButton() {
    ...
    if (saveBtn) {
        ...
        saveBtn.style.boxShadow = '0 0 20px rgba(255, 30, 39, 0.8)';
        setTimeout(() => {
